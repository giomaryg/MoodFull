  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-4 pb-4 border-b border-[#e8d5c4]">
        <div>
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#c17a7a] flex items-center gap-2 sm:gap-3">
            <ChefHat className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8" />
            Your Recipe Options
          </h2>
          <p className="text-[#9b8175] mt-1 text-sm sm:text-base">Found {filteredAndSortedRecipes.length} recipe{filteredAndSortedRecipes.length !== 1 ? 's' : ''} for you</p>
        </div>

        <div className="flex flex-wrap gap-2 sm:gap-3 items-center">
          {/* Filter by difficulty */}
          <div className="flex items-center gap-2 flex-1 sm:flex-initial">
            <Filter className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#9b8175]" />
            <Select value={filterDifficulty} onValueChange={setFilterDifficulty}>
              <SelectTrigger className="flex-1 sm:w-32 md:w-36 border-[#e8d5c4] text-sm">
          ...
          {/* Sort */}
          <div className="flex items-center gap-2 flex-1 sm:flex-initial">
            <ArrowUpDown className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#9b8175]" />
            <Select value={sortBy} onValueChange={setSortBy}>
              <SelectTrigger className="flex-1 sm:w-36 md:w-40 border-[#e8d5c4] text-sm">
          ...
          <Button
            onClick={onStartOver}
            variant="outline"
            className="border-2 border-[#c17a7a] hover:border-[#b06a6a] hover:bg-[#f5e6dc] text-[#c17a7a] text-sm w-full sm:w-auto"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
        {filteredAndSortedRecipes.map((recipe, index) => (
          ...
          >
            ...
            >
              {/* Recipe Image */}
              <div className="relative h-40 sm:h-44 md:h-48 bg-gradient-to-br from-[#f5e6dc] to-[#e8d5c4] overflow-hidden">
                {recipe.imageLoading ? (
              ...
              <CardContent className="p-4 sm:p-5 md:p-6 space-y-3 sm:space-y-4 flex flex-col">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-base sm:text-lg md:text-xl text-gray-900 group-hover:text-[#c17a7a] transition-colors line-clamp-2 leading-tight">
                    {recipe.name}
                  </h3>
                ...
                {/* Metadata */}
                <div className="space-y-2 sm:space-y-3 pt-2 sm:pt-3 border-t border-[#e8d5c4]">
                  <div className="flex flex-wrap gap-1.5 sm:gap-2">
                    ...
                    {recipe.cook_time && (
                    <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-[#9b8175]">
      ...
      {filteredAndSortedRecipes.length === 0 && (
        <div className="text-center py-8 sm:py-12">
          <p className="text-[#9b8175] text-sm sm:text-base md:text-lg px-4">No recipes match your filters. Try adjusting your selection.</p>
        </div>
      )}
    </div>
  );
